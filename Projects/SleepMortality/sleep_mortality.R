...
options(stringsAsFactors = FALSE, digits = 3, width = 160)
library(scales)
library(quantreg)
...
dwuse <- dwclean[!is.na(LastAge) & (!is.na(B1Sleep) | !is.na(C1Sleep)) & LastAge > AGEM2, .(
  B1Sleep, C1Sleep, AGEM1, AGEM2, AGEM3, LastAge, DDate, DECEASED,
  M2ID, M2FAMNUM, RaceG3, Sex, B4PSQI1, B1WeekdaySleep, B1WeekendSleep,
  C1WeekdaySleep, C1WeekendSleep, B1SleepOnsetLatency, C1SleepOnsetLatency,
  B1SSQ, C1SSQ,
  A1SCHRON, B1SCHRON, C1SCHRON,
  A1SWSTHI, B1SWSTHI, C1SWSTHI,
  A1SBMI, B1SBMI, C1SBMI, B1Smoke, B1CurrentAlcohol, m2ed_all)]

## dwuse[, .(B1Sleep, C1Sleep, AGEM1, B1PAGE_M2, C1PRAGE, LastAge, DDate, DECEASED)][(C1PRAGE - B1PAGE_M2) < 7]

dwuse[, B1Age := (AGEM2 - 30) / 10]
dwuse[, C1Age := (AGEM3 - 30) / 10]
dwuse[, White := as.integer(RaceG3 == "White")]
dwuse[, AA := as.integer(RaceG3 == "AA")]
dwuse[, SmokeCurr := as.integer(B1Smoke == "current")]
dwuse[, SmokePast := as.integer(B1Smoke == "past")]
dwuse[, AlcHigh := as.integer(B1CurrentAlcohol == "High")]
dwuse[, AlcMod := as.integer(B1CurrentAlcohol == "Moderate")]

xtabs(~is.na(B1Sleep) + is.na(B1PAGE_M2), data = dwuse)
xtabs(~is.na(C1Sleep) + is.na(C1PRAGE), data = dwuse)

summary(dwclean[, .(Diff = C1PRAGE - B1PAGE_M2)]$Diff)

hist(dwuse$B1Sleep)
hist(dwuse$C1Sleep)
hist(winsorizor(sqrt(dwuse$B1SleepOnsetLatency), .005))
hist(winsorizor(sqrt(dwuse$C1SleepOnsetLatency), .005))

dwuse[, B1SOL := (winsorizor(sqrt(dwuse$B1SleepOnsetLatency), .005) - .5) * 60]
dwuse[, C1SOL := (winsorizor(sqrt(dwuse$C1SleepOnsetLatency), .005) - .5) * 60]
dwuse[, B1Sleep := B1Sleep - 7.5]
dwuse[, C1Sleep := C1Sleep - 7.5]
dwuse[, dSleep := C1Sleep - B1Sleep]
dwuse[, dSOL := (((C1SOL/60)+.5)^2 - ((B1SOL/60)+.5)^2) * 60]
dwuse[, dAge := (C1Age - B1Age) * 10]

dwuse[, dSOLCat := cut(dSOL, breaks = c(-Inf, -30, -20, -10, -.0001, 0, 10, 20, 30, Inf),
                       labels = c("30+ min", "30 - 20 min", "20 - 10 min",
                                  "10 - 0 min", "No Change", "0 - 10 min", "10 - 20 min",
                                  "20 - 30 min", "30+ min "))]
dwuse[, dSleepCat := cut(dSleep, breaks = c(-Inf, -2, -1.5, -1, -.5, -.0001, 0, .5, 1, 1.5, 2, Inf),
                       labels = c("2+ hours", "120 - 90 min", "90 - 60 min",
                                  "60 - 30 min", "30 - 0 min", "No Change",
                                  "0 - 30 min", "30 - 60 min", "60 - 90 min",
                                  "90 - 120 min", "2+ hours "))]
dwuse[, dSOLCat2 := cut(dSOL, breaks = c(-Inf, -.0001, 0, Inf),
                        labels = c("Shorter", "No Change", "Longer"))]
dwuse[, dSleepCat2 := cut(dSleep, breaks = c(-Inf, -.0001, 0, Inf),
                        labels = c("Shorter", "No Change", "Longer"))]

tmpl <- reshape(dwuse[, .(B1Sleep, C1Sleep, B1SOL, C1SOL, B1Age, C1Age, dSleep, M2ID)],
                varying = list(c("B1Sleep", "C1Sleep"),
                               c("B1SOL", "C1SOL"),
                       c("B1Age", "C1Age")),
        v.names = c("Sleep", "SOL", "Age"),
        timevar = "time",
        idvar = "M2ID",
        direction = "long")

tmpl[, Age := (Age * 10) + 30]
tmpl[, Sleep := Sleep + 7.5]
tmpl[, SOL := (((SOL/60) + .5)^2) * 60]
tmpl[, Age2 := Age - min(Age, na.rm = TRUE), by = M2ID]
tmpl[, AgeCat := cut(Age, quantile(Age, probs = seq(0, 1, by = .05), na.rm=TRUE), include.lowest = TRUE)]

## library(splines)
## mrq.sleep <- rq(Sleep ~ bs(Age, df = 10), tau = c(.5), data = tmpl)
## mrq.sleep <- rq(Sleep ~ Age + I(Age^2), tau = c(.1), data = tmpl)
## mrq.sleep <- rq(Sleep ~ Age + I(Age^2), tau = c(.9), data = tmpl)
## summary(mrq.sleep)
## ggplot(tmpl, aes(Age, Sleep)) +
##   stat_smooth(method = "lm", colour = rgb(0, 108, 171, maxColorValue = 255),
##               fill = "grey90", alpha = 1, formula = y ~ x + I(x^2)) +
##   stat_quantile(formula = y ~ x + I(x^2), quantiles = c(.1, .25, .5, .75, .9),
##                 size = 1, colour = rgb(133, 219, 230, maxColorValue = 255))
## ggplot(tmpl, aes(Age, Sleep)) +
##   stat_quantile(formula = y ~ bs(x, df = 15), quantiles = c(.1, .25, .5, .75, .9),
##                 size = 1, colour = rgb(133, 219, 230, maxColorValue = 255))
##   stat_smooth(method = "loess", se=FALSE, colour = rgb(133, 219, 230, maxColorValue = 255)) +
##   scale_x_continuous("Age (years)", breaks = seq(30, 90, by = 10)) +
##   scale_y_continuous("Sleep Duration (hours)", breaks = seq(7.1, 7.7, by = .1))


p.sleeptraj <- ggplot(tmpl, aes(Age, Sleep)) +
  geom_quantile(method = "rq", colour = rgb(253, 180, 21, maxColorValue = 255),
              formula = y ~ x + I(x^2), size = 1.5,
              quantiles = c(.1, .5, .9)) +
  ## stat_smooth(method = "lm", colour = rgb(253, 180, 21, maxColorValue = 255),
  ##             fill = "grey90", alpha = 1, formula = y ~ x + I(x^2), size = 1.5) +
  ## stat_smooth(method = "lm", colour = rgb(0, 108, 171, maxColorValue = 255),
  ##             fill = "grey90", alpha = 1, formula = y ~ x + I(x^2)) +
  ##stat_smooth(method = "loess", se=FALSE, colour = rgb(133, 219, 230, maxColorValue = 255)) +
  scale_x_continuous("Age (years)", breaks = seq(30, 90, by = 10)) +
  scale_y_continuous("Sleep Duration (hours)") +
  coord_cartesian(ylim = c(5, 10), expand = FALSE)

p.sleepchange <- ggplot(dwuse[!is.na(dSleepCat)], aes(dSleepCat, fill = dSleepCat2)) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  ## scale_fill_manual("",
  ##                   values = c("Shorter" = rgb(0, 108, 171, maxColorValue = 255),
  ##                              "No Change" = "grey50",
  ##                              "Longer" = rgb(133, 219, 230, maxColorValue = 255))) +
  scale_fill_manual("",
                    values = c("Shorter" = rgb(242, 98, 34, maxColorValue = 255),
                               "No Change" = "grey50",
                               "Longer" = rgb(253, 180, 21, maxColorValue = 255))) +
  scale_y_continuous("Percent of Participants", labels = percent) +
  xlab("Change in Sleep Duration") +
  coord_cartesian(ylim = c(0, .25), xlim = c(.5, 11.5), expand=FALSE) +
  theme(legend.justification=c(0,1), legend.position=c(0,1),
        axis.text.x = element_text(angle = 45, hjust= 1, vjust = 1))

p.soltraj <- ggplot(tmpl, aes(Age, SOL)) +
  geom_quantile(method = "rq", colour = rgb(97, 190, 173, maxColorValue = 255),
              formula = y ~ x + I(x^2), size = 1.5,
              quantiles = c(.1, .5, .9)) +
  ## stat_smooth(method = "lm", colour = rgb(97, 190, 173, maxColorValue = 255),
  ##             fill = "grey90", alpha = 1, formula = y ~ x + I(x^2)) +
  ## stat_smooth(method = "lm", colour = rgb(0, 108, 171, maxColorValue = 255),
  ##             fill = "grey90", alpha = 1, formula = y ~ x + I(x^2)) +
  ##stat_smooth(method = "loess", se=FALSE, colour = rgb(133, 219, 230, maxColorValue = 255)) +
  scale_x_continuous("Age (years)", breaks = seq(30, 90, by = 10)) +
  scale_y_continuous("Sleep Onset Latency (minutes)", breaks = c(0, 20, 40, 60)) +
  coord_cartesian(ylim = c(0, 65), expand = FALSE)

p.solchange <- ggplot(dwuse[!is.na(dSOLCat)], aes(dSOLCat, fill = dSOLCat2)) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  scale_fill_manual("",
                    values = c("Shorter" = rgb(97, 190, 173, maxColorValue = 255),
                               "No Change" = "grey50",
                               "Longer" = rgb(98, 148, 176, maxColorValue = 255))) +
  ## scale_fill_manual("",
  ##                   values = c("Shorter" = rgb(133, 219, 230, maxColorValue = 255),
  ##                              "No Change" = "grey50",
  ##                              "Longer" = rgb(0, 108, 171, maxColorValue = 255))) +
  scale_y_continuous("Percent of Participants", labels = percent) +
  xlab("Change in Sleep Onset Latency") +
  coord_cartesian(ylim = c(0, .25), xlim = c(.5, 9.5), expand=FALSE) +
  theme(legend.justification=c(0,1), legend.position=c(0,1),
        axis.text.x = element_text(angle = 45, hjust= 1, vjust = 1))

save_plot("~/OneDrive/Presentations/2017/APSS/sleep_desc_duration.emf",
  plot_grid(p.sleeptraj, p.sleepchange,
          ncol = 2,
          align = "hv"), ncol = 2,
  base_height = 4, base_aspect_ratio = 1.2)

save_plot("~/OneDrive/Presentations/2017/APSS/sleep_desc_quality.emf",
  plot_grid(p.soltraj, p.solchange,
          ncol = 2,
          align = "hv"), ncol = 2,
  base_height = 4, base_aspect_ratio = 1.2)


save_plot("~/OneDrive/Presentations/2017/APSS/sleep_desc.emf",
  plot_grid(p.sleeptraj, p.sleepchange,
          p.soltraj, p.solchange,
          ncol = 2,
          align = "hv",
          labels = c("A", "B", "C", "D")), ncol = 2,
  base_height = 8, base_aspect_ratio = .6)

ggsave("~/OneDrive/Presentations/2017/APSS/age_sleepdur.emf", plot = p.sleeptraj,
       width = 5, height = 3)

ggplot(tmpl, aes(Age2, Sleep, group = M2ID)) +
  geom_line(alpha=.2) +
  stat_smooth(aes(group=1), method = "loess", se=FALSE, colour = "blue")

ggplot(tmpl, aes(as.integer(AgeCat), Sleep)) +
  stat_summary(fun.data = mean_cl_boot) +
  stat_smooth(method = "lm", colour = "black", formula = y ~ x + I(x^2)) +
  facet_wrap(~cut(dSleep, c(-Inf, -.5, .5, Inf)))
...
m <- mplusObject(
  ...
  PROCESSORS = 2;",
  MODEL = "
  i s q | B1Sleep C1Sleep at B1Age C1Age;
  B1Sleep C1Sleep (e);
  i WITH s* q*;
  i* s* q*;",
  rdata = as.data.frame(dwuse))
m.res <- mplusModeler(m, dataout = "m_v1.dat", run = TRUE)
...
m <- mplusObject(
  ...
  i* s* q*;",
  rdata = as.data.frame(dwuse))
m.res <- mplusModeler(m, dataout = "m_v2.dat", run = TRUE)
...
m2 <- mplusObject(
  ...
  ",
  rdata = as.data.frame(dwuse))
m2.res <- mplusModeler(m2, dataout = "m2_v1.dat", run = TRUE)

library(GGally)

m.over <- survfit(Surv(LastAge, DECEASED) ~ 1, data = dwuse)

ggsave("~/OneDrive/Presentations/2017/APSS/survplot.emf",
  ggsurv(m.over, plot.cens = FALSE, surv.col = rgb(0, 108, 171, maxColorValue = 255),
       CI=FALSE, size.est = 2, main = "Overall Survival Curve") +
  scale_x_continuous("Age (years)", breaks = c(30, 40, 50, 60, 70, 80, 90)) +
  scale_y_continuous("Survival", labels = percent) +
  coord_cartesian(xlim = c(30, 95), ylim = c(0, 1), expand = FALSE),
  width = 5, height = 4)

ggsave("~/OneDrive/Presentations/2017/APSS/survplot2.emf",
  ggsurv(m.over, plot.cens = FALSE, surv.col = rgb(238, 50, 36, maxColorValue = 255),
       CI=FALSE, size.est = 2, main = "Overall Survival Curve") +
  scale_x_continuous("Age (years)", breaks = c(30, 40, 50, 60, 70, 80, 90)) +
  scale_y_continuous("Survival", labels = percent) +
  coord_cartesian(xlim = c(30, 95), ylim = c(0, 1), expand = FALSE),
...
summary(coxph(Surv(LastAge, DECEASED) ~ cut(dSleep / dAge, breaks = c(-Inf, -.556, .556, Inf)),
              data = dwuse))



summary(coxph(Surv(LastAge, DECEASED) ~ I(dSleep>-.5 & dSleep < .5) + I(dSleep >= .5), data = dwuse))

summary(coxph(Surv(LastAge, DECEASED) ~ B1Sleep, data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ B1Sleep + I(B1Sleep^2), data = dwuse))

summary(coxph(Surv(LastAge, DECEASED) ~ cut(B1Sleep + 7.5, breaks = c(0, 5, 6, 7, 8, 9, 24)), data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ cut(B1Sleep + 7.5, breaks = c(0, 6, 9.5, 24)), data = dwuse))

summary(coxph(Surv(LastAge, DECEASED) ~ cut(C1Sleep, breaks = c(0, 5, 6, 7, 8, 9, 24)), data = dwuse))

summary(coxph(Surv(LastAge, DECEASED) ~ factor(Sex) + factor(Sex):C1SOL, data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ B1WeekdaySleep, data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ B1WeekendSleep, data = dwuse))

summary(coxph(Surv(LastAge, DECEASED) ~ B1SOL, data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ C1SOL, data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ B1SOL + I(C1SOL-B1SOL), data = dwuse))
summary(coxph(Surv(LastAge, DECEASED) ~ Sex * (B1SOL + C1SOL), data = dwuse))

## tmplong <- na.omit(dwuse[, .(M2ID, B1Age, C1Age, B1SOL, C1SOL)])
## tmplong <- melt(tmplong, measure = list(age = c("B1Age", "C1Age"), SOL = c("B1SOL", "C1SOL")), id.vars = "M2ID")
## library(lme4)
## summary(mres <- lmer(scale(value2) ~ 1 + scale(value1) + (1 | M2ID) + (0 + scale(value1) | M2ID), data = droplevels(na.omit(tmplong))))
## summary(mres <- lmer(scale(value2) ~ 1 + scale(value1) + (1 + scale(value1) | M2ID), data = tmplong))
## slopest <- cbind(M2ID = unique(droplevels(na.omit(tmplong))$M2ID), ranef(mres)$M2ID)
## names(slopest) <- c("M2ID", "U_SOL", "S_SOL")
## slopest <- merge(slopest, dwuse, by = "M2ID", all=TRUE)
## summary(coxph(Surv(LastAge, DECEASED) ~ S_SOL, data = slopest))
...
rdata = as.data.frame(dwuse),
usevariables = c("M2ID", "M2FAMNUM", "B1Age", "C1Age", "LastAge", "DECEASED", "B1Sleep", "C1Sleep"))
m3a.res <- mplusModeler(m3a, dataout = "m3a_v1.dat", run = TRUE)
screenreg(m3a.res, single.row = TRUE, cis=TRUE, digits = 3)
screenreg(m3a.res, type = "stdyx", single.row = TRUE, cis=TRUE, digits = 3)
...
rdata = as.data.frame(dwuse),
usevariables = c("M2ID", "M2FAMNUM", "B1Age", "C1Age", "LastAge", "DECEASED", "B1Sleep", "C1Sleep",
                 "Sex", "B1SBMI", "B1SCHRON", "White", "SmokeCurr", "SmokePast", "AlcHigh",
                 "AlcMod", "m2ed_all"))
m3b.res <- mplusModeler(m3b, dataout = "m3b_v1.dat", run = TRUE)
screenreg(m3b.res, single.row = TRUE, cis=TRUE, digits = 3)
...
OUTPUT = "CINTERVAL;",
rdata = as.data.frame(dwuse),
usevariables = c("M2ID", "M2FAMNUM", "B1Age", "C1Age", "LastAge", "DECEASED", "B1SOL", "C1SOL"))
m4a.res <- mplusModeler(m4a, dataout = "m4a_v1.dat", run = TRUE)
screenreg(m4a.res, single.row = TRUE, cis=TRUE)
...
m4b.res <- mplusModeler(m4b, dataout = "m4b_v1.dat", run = TRUE)
screenreg(m4b.res, single.row = TRUE, cis=TRUE)
...
usevariables = c("M2ID", "M2FAMNUM", "B1Age", "C1Age", "LastAge", "DECEASED", "B1SOL", "C1SOL",
                 "Sex", "B1SBMI", "B1SCHRON", "White", "SmokeCurr", "SmokePast", "AlcHigh",
                 "AlcMod", "m2ed_all", "B1Sleep"))

m4c.res <- mplusModeler(m4c, dataout = "m4c_v1.dat", run = TRUE)
screenreg(m4c.res, single.row = TRUE, cis=TRUE)
...
m4c <- mplusObject(
  ...
  LastAge ON i s;
  LastAge i s ON Sex B1SBMI B1SCHRON C1SCHRON White SmokeCurr SmokePast AlcHigh AlcMod;
  [Sex B1SBMI B1SCHRON C1SCHRON White SmokeCurr SmokePast AlcHigh AlcMod];
",
  rdata = as.data.frame(dwuse))
m4c.res <- mplusModeler(m4c, dataout = "m4c_v1.dat", run = TRUE)
...
",
  rdata = as.data.frame(dwuse))
m5.res <- mplusModeler(m5, dataout = "m5_v1.dat", run = TRUE)
...
",
  rdata = as.data.frame(dwuse))
m4c2.res <- mplusModeler(m4c2, dataout = "m4c2_v1.dat", run = TRUE)

xtabs(~AA + Sex, data = na.omit(dwuse[, .(LastAge, Sex, B1SBMI, B1SCHRON, AA, White, SmokeCurr, SmokePast, AlcHigh, AlcMod, m2ed_all, DECEASED)]))
...
",
  rdata = as.data.frame(dwuse))
m4c2.res <- mplusModeler(m4c2, dataout = "m4c2_v2.dat", run = TRUE)
...
m4c2 <- mplusObject(
  ...
  LastAge ON i s;
  LastAge i s ON Sex B1SBMI B1SCHRON AA White SmokeCurr SmokePast AlcHigh AlcMod;
",
  rdata = as.data.frame(dwuse))
m4c2.res <- mplusModeler(m4c2, dataout = "m4c2_v1.dat", run = TRUE)
...
m4a <- mplusObject(
  ...
  LastAge ON i s;
",
  rdata = as.data.frame(dwuse[Sex == 1]))
m4a.res <- mplusModeler(m4a, dataout = "m4a_v2.dat", run = TRUE)
...
m4b <- mplusObject(
  ...
  LastAge ON i s;
",
  rdata = as.data.frame(dwuse[Sex == 2]))
m4b.res <- mplusModeler(m4b, dataout = "m4b_v1.dat", run = TRUE)
...
",
  rdata = as.data.frame(dwuse))
m3.res <- mplusModeler(m3, dataout = "m3_v1.dat", run = TRUE)
...
",
  rdata = as.data.frame(dwuse[Sex==1]))
m3a.res <- mplusModeler(m3a, dataout = "m3a_v2.dat", run = TRUE)
...
",
  rdata = as.data.frame(dwuse[Sex==2]))
m3b.res <- mplusModeler(m3b, dataout = "m3b_v2.dat", run = TRUE)
...
dwcleanimp <- readRDS("~/OneDrive/Projects/MIDUS-Setup/midus_rcm_final_imputed.RDS")
dwclean <- readRDS("~/OneDrive/Projects/MIDUS-Setup/midus_merged_data_dwclean.RDS")
...
dw <- readRDS("midus_merged_data_dwclean.RDS")
