...
dw <- readRDS("~/OneDrive/Projects/MIDUS-Setup/midus_merged_data_dwclean.RDS")
dwl <- readRDS("~/OneDrive/Projects/MIDUS-Setup/midus_merged_data_dwl.RDS")
...
stress.labs <- data.table(
  levels = c("A1SDAYDI", "A1SLFEDI", "B1ChildLifeStress",
             ...
             "A1SHOMET", "A1ParentAbuse", "A1SiblingAbuse"),
...
dimputedl <- as.data.table(do.call(rbind, lapply(1:50, function(i) {
  ...
  ## out1 <- merge(cbind(dimputed[[i]], Imp = i, M2ID = sdat$M2ID), tmp.iiv2, by = "M2ID", all.x = TRUE)
  ## out1 <- merge(cbind(sdatimp, Imp = i, M2ID = sdat$M2ID), tmp.iiv2, by = "M2ID", all.x = TRUE)
  ## merge(out1, dw[, .(M2ID, M2FAMNUM)], by = "M2ID", all.x = TRUE)

  merge(cbind(as.data.table(snimp[[i]])[M2ID %in% d.iiv$TST$d$M2ID], Imp = i), tmp.iiv2, by = "M2ID", all = TRUE)
})))

dimputedl[, A1SLFEDI := pmin(A1SLFEDI, 3)]
...
clusterExport(cl, c("stress.vars", "dimputed", "pool.r2.gls", "r2.gls"))
clusterEvalQ(cl, {
  library(mice)
  library(nlme)
  library(data.table)
...
## sleep.vars
m.linear <- lapply(c("U_TST", "U_BT", "U_RT", "U_SOL", "U_SE", "U_WASO", "V_TST", "V_BT", "V_RT", "V_SOL", "V_SE", "V_WASO"), function(dv) {
  lapply(stress.vars, function(iv) {
    f1 <- sprintf("scale(%s) ~ Sex + AGEM2 + I(RaceG3=='White') + m1chadv + A1SCHRON + A1DepAnx + scale(%s)",
                 dv, iv)
    f2 <- sprintf("scale(%s) ~ Sex + AGEM2 + I(RaceG3=='White') + m1chadv + A1SCHRON + A1DepAnx",
                 dv)

    f1 <- sprintf("scale(%s) ~ scale(%s)",
                  dv, iv)
    f2 <- sprintf("scale(%s) ~ 1",
                  dv)

    ## usedat <- na.omit(model.frame(as.formula(f), data = sdat))
    m <- parLapplyLB(cl, dimputed, function(usedat) {
      ## usedat <- na.omit(model.frame(as.formula(f1)), data = usedat)
      m1 <- tryCatch(gls(as.formula(f1), data = usedat, correlation = corCompSymm(form = ~ 1 | M2FAMNUM), na.action = na.omit),
                     error = function(e) e)
      ## m1 <- tryCatch(gls(as.formula(f1), data = usedat, na.action = na.omit),
      ##                error = function(e) e)
      if (inherits(m1, "error")) {
        m1 <- NULL
      }
      m2 <- tryCatch(gls(as.formula(f2), data = usedat, correlation = corCompSymm(form = ~ 1 | M2FAMNUM), na.action = na.omit),
                     error = function(e) e)
      ## m2 <- tryCatch(gls(as.formula(f2), data = usedat, na.action = na.omit),
      ##                error = function(e) e)
    ...
    r2 <- data.table(
      DV = dv, IV = iv,
      CovR2 = pool.r2.gls(m, 2),
      AllR2 = pool.r2.gls(m, 1),
...
m.linear.coef[, DV := factor(DV,
                             levels = c("U_BT", "U_RT", "U_TST", "U_SOL", "U_SE", "U_WASO",
                                        "V_BT", "V_RT", "V_TST", "V_SOL", "V_SE", "V_WASO"),
