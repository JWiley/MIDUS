...
m.linear <- lapply(c("1", "1 + Sex + AGEM2 + I(RaceG3=='White') + A1SCHRON + A1DepAnx"), function(covs) {
  lapply(c("U_TST", "U_BT", "U_RT", "U_SOL", "U_SE", "U_WASO",
           "V_TST", "V_BT", "V_RT", "V_SOL", "V_SE", "V_WASO"), function(dv) {
  lapply(stress.vars, function(iv) {
    f1 <- sprintf("scale(%s) ~ %s + scale(%s)", dv, covs, iv)
...
m.linear.coef <- as.data.table(do.call(rbind, lapply(m.linear[[2]], function(d) do.call(rbind, lapply(d, function(x) x$B)))))

setnames(m.linear.coef, old = names(m.linear.coef), c("DV", "IV", "B", "SE", "T", "DF", "P", "LL", "UL", "NMIS", "FMI", "Lambda"))

m.linear.r2 <- as.data.table(do.call(rbind, lapply(m.linear[[2]], function(d) do.call(rbind, lapply(d, function(x) x$R2)))))
...
p.mean <- ggplot(droplevels(m.linear.coef[!grepl("IIV", DV)]), aes(DV, IV, fill = StrR2*100)) +
  geom_tile() +
...
p.iiv <- ggplot(droplevels(m.linear.coef[grepl("IIV", DV)]), aes(DV, IV, fill = StrR2*100)) +
  geom_tile() +
  scale_fill_gradient(name = expression(R^2), low = "#ffffff", high = "#0079AA", limits = c(0, 4)) +
  geom_text(aes(label = gsub("0\\.", ".", sprintf("%0.2f%s", B,
    symnum(P, legend = FALSE, na = "", cutpoints = c(0, 0.001,
