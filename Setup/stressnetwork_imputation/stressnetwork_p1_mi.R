...
if (aws) {
  checkpoint("2018-01-01", R.version = "3.4.3", checkpointLocation = "~/EncDB",
             use.knitr=TRUE)
} else {
  checkpoint("2018-01-01", R.version = "3.4.3", use.knitr = TRUE)
}
...
if (aws) {
  ...
  psr_pana_bi.data <- readRDS("~/OneDrive/Projects/MIDUS/Data/Processed/midus_merged_data_psrpanabi_imp.RDS")
}
...
useIDs <- useIDs[useIDs %in% dwcleanimp[[1]][!is.na(LastAge), M2ID]]
length(useIDs) ## after excluding missing last age
...
#  dwcleanimp[[i]][, B4S9 := factor(as.integer(B4S9 == 4))]
  dwcleanimp[[i]][, c(v$cov.cat, v$health.cat, "DECEASED") := lapply(.SD, as.factor), .SDcols = c(v$cov.cat, v$health.cat, "DECEASED")]
}
...
if (TRUE) {
  ...
  if (!aws) {
    ...
    clusterEvalQ(cl, {
      library(checkpoint)
      checkpoint("2018-01-01", R.version = "3.4.3", scanForPackages = FALSE)
      library(mice)
      library(randomForest)
      ## library(caret)
      ## library(ipred)
...
imputation_seeds <- c(
  ...
  noimpdex <- which(colnames(dwcleanimp[[1]]) %in% c("M2ID", "M2FAMNUM", "LastAge"))
  ...
  if (aws) {
    imputed <- mclapply(X = 1:50, FUN = function(i) {
      ...
      saveRDS(out, file = sprintf("midus_stress_imputed_%s.RDS", as.character(i)))
      return(out)
    }, mc.cores = 4)
    ...
    imputed <- parLapplyLB(cl, 1:50, function(i) {
      ...
      saveRDS(out, file = sprintf("midus_stress_imputed_%s.RDS", as.character(i)), compress = "xz")
      return(out)
    })
  }

  saveRDS(imputed, file = "midus_stress_imputed.RDS", compress = "xz")

  final_imputed <- lapply(imputed, mice::complete, 1)

saveRDS(final_imputed, file = "midus_stress_final_imputed.RDS", compress = "xz")
}


# imputed <- lapply(1:50, function(i) {
#   readRDS(sprintf("midus_stress_imputed_%d.RDS", i))
# })
